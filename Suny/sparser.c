...
(struct Sparser *parser) {
    ...
    node->ops = Smem_Calloc(MAX_STATEMENT_SIZE, sizeof(enum Stok_t));
...
(struct Sparser *parser) {
    ...
    node->else_body = Sast_init(AST_BLOCK, 0, NULL);
    ...
    while (ptoken(parser)->type == ELIF) {
        ...
        elif->else_body = Sast_init(AST_BLOCK, 0, NULL);
    ...
    if (ptoken(parser)->type == ELSE) {
        ...
        while (ptoken(parser)->type != END) {
            struct Sast *stmt = Sparser_parse(parser);
            Sast_set_line(parser->lexer, stmt);
