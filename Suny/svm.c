...
(struct Sframe *frame) {
    ...
    if (TypeOf(list) == LIST_OBJ) {
    ...
    } else if (TypeOf(list) == STRING_OBJ) {
        ...
        struct Sobj *obj = Sobj_make_char(Sstr_get_char(tget_str(list), index_value));
        ...
        Sframe_push(frame, obj);
    } else if (TypeOf(list) == USER_DATA_OBJ) {
    ...
    } else if (TypeOf(list) == MAP_OBJ) {
        struct Shashmap* map = tget_hashmap(list);

        struct Sobj* value = Shashmap_get(map, index);
...
(struct Sframe *frame) {
    ...
    } else if (TypeOf(list) == LIST_OBJ) {
        struct Slist *slist = list->f_type->f_list;
        int index_value = index->value->value;
        Slist_store(frame, slist, index_value, value);
    } else if (TypeOf(list) == MAP_OBJ) {
        struct Shashmap* map = tget_hashmap(list);
        Shashmap_store_pair(frame, map, index, value);
...
(struct Sframe *frame) {
    ...
    double size = Slen(obj);

    Sframe_push_number(frame, size);
...
(struct Sframe *frame) {
    ...
    int value = Slogic_AND(obj1, obj2);
...
(struct Sframe *frame) {
    SDEBUG("SUNY_API struct Sframe* Svm_evaluate_MAKE_MAP(struct Sframe *frame) building...");
    ...
    struct Sobj* obj = Sobj_make_hashmap(frame, lkeys, lvalues);

    if (!obj) Sframe_push_null(frame);
    else Sframe_push(frame, obj);

    SDEBUG("SUNY_API struct Sframe* Svm_evaluate_MAKE_MAP(struct Sframe *frame) done");
