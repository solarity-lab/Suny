...
struct Sast* Sast_new(void) {
    ...
    sast->super_class_count = 0;
    sast->super_class_names[0] = NULL;
    ...
    sast->condition = NULL;
    sast->if_body = Smem_Calloc(MAX_STATEMENT_SIZE, sizeof(struct Sast *));
    sast->else_body = Smem_Calloc(MAX_STATEMENT_SIZE, sizeof(struct Sast *));
    sast->if_body_size = 0;
