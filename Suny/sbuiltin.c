...
SUNY_API struct Sobj* Ssize(struct Sframe* frame) {
    ...
    if (TypeOf(obj) == LIST_OBJ) {
        MOVETOGC(obj, frame->gc_pool);
        return Svalue(tget_list(obj)->count);
    } else if (TypeOf(obj) == STRING_OBJ) {
        MOVETOGC(obj, frame->gc_pool);
        return Svalue(tget_str(obj)->size);
    }
...
SUNY_API struct Sobj* Spop(struct Sframe* frame) {
    ...
    struct Slist* list = tget_list(obj);
...
SUNY_API struct Sobj* Stype(struct Sframe* frame) {
    ...
    } else if (TypeOf(obj) == NUMBER_OBJ) {
        cast = Sobj_make_string("number", 6);
    } else if (TypeOf(obj) == TRUE_OBJ || TypeOf(obj) == FALSE_OBJ) {
